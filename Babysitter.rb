  ...
  def charge_in_dollars(shift)
    ...
    while time < shift.end_time
      if time >= shift.midnight
        charge_in_dollars += @hourly_rate_after_midnight
      elsif time >= shift.bedtime
        charge_in_dollars += @hourly_rate_from_bedtime_to_midnight
      else
        charge_in_dollars += @hourly_rate_before_bedtime
      end
